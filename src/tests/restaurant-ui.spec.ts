...
describe('UI tests', () => {
    ...
    it('Add, Verify, Delete', async function () {
        const page = new RestaurantPage(browser);
        const popup = await page.openCreateRestaurantPopup();
        await popup.init();
        const actualTitle = await popup.getTitle();
        const expectedTitle = "Create new restaurant";
        expect(actualTitle).to.equal(expectedTitle, 'Restaurants popup was not opened')
        await page.fillTheNewRestPopUp("123", "frog", "Barca", "9.8");
        await page.clickOnAccept()
        await page.checkSucceedPopUp()
        expect(await page.ifMyRestInPage()).to.true
        await page.deleteMyRest()
        await page.checkSucceedPopUp()
        expect(await page.validationRestIDNotInTable()).to.true
    })
})
